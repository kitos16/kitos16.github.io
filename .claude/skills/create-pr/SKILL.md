...
3. **Ejecuta `npm run build` para validar que el build pasa** - Si falla, muestra el error al usuario y no continues
4. Haz push de la branch si no existe en remoto: `git push -u origin <branch>`
5. Obtén el branch name actual: `git rev-parse --abbrev-ref HEAD`
6. Obtén el ID y título de la tarea desde PLAN.md (busca la línea con la branch actual)
7. Crea el PR usando gh con:
